...
export default function EmailVerification({ onAfterEmailVerification }: { onAfterEmailVerification: () => void }) {
    const [step, setStep] = useState<'email' | 'otp' | 'success'>('email')
    const [email, setEmail] = useState('')
    const [otpError, setOtpError] = useState<string | null>(null)
    const { setAccessToken } = useAuthStore()
    const { setTokenExpiry } = useGlobalStateStore()

    const { mutate: sendOTPMutation, isPending } = useMutation({
        mutationFn: () =>
            apiAuth.post(endpoints.players.sendOtpForPlayerRegistration, {
                email: email,
            }),
        onSuccess: () => {
            setStep('otp')
    ...
    const { mutate: verifyOTPMutation } = useMutation({
        mutationFn: (values: OTPFormValues) =>
            apiAuth.post(endpoints.players.verifyOtpForPlayerRegistration, {
                email: email,
                otp: values.otp,
            }),
        onSuccess: (res: AxiosResponse) => {
            const accessToken = res.data.data.accessToken
            const tokenExpiry = Number(res.data.data.tokenExpireWithin.replace('m', ''))
            if (accessToken) {
                    ...
                    {step === 'email' && (
                        <Form {...emailForm}>
                            <form onSubmit={emailForm.handleSubmit(handleSendOTP)} className="space-y-6">
                                ...
                                />
                                <Button type="submit" className="w-full" disabled={emailForm.formState.isSubmitting || isPending}>
                                    {emailForm.formState.isSubmitting || isPending ? 'Sending OTP...' : 'Send OTP'}
                                </Button>
                            </form>
                        </Form>
                    )}
                                        ...
                                        <FormItem>
                                            ...
                                            {otpError && (
                                                <Alert className="mt-2 border-red-500 bg-red-50">
                                                    ...
                                                    <AlertDescription className="text-red-800">{otpError}</AlertDescription>
                                                </Alert>
                                            )}
